...
Job.prototype.updateDelay = async function(delayTimestamp) {
  const code = await scripts.updateDelay(this, delayTimestamp);

  switch (code) {
    case -1:
      throw new Error('Job does not exist', this.id);
    case -2:
      throw new Error('Job is not in delayed set', this.id);
    case -3:
      throw new Error('Invalid delay timestamp for job ' + this.id);
  }
  if (code === 0) this.delay = delayTimestamp - this.timestamp;
};
