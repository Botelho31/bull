...
const scripts = {
  ...
  updateDelay(job, delayTimestamp) {
    const queue = job.queue;
    const queueKeys = queue.keys;
    const keys = [queueKeys.delayed];

    const args = [job.id, queueKeys[''], delayTimestamp.toString(), job.timestamp.toString()];

    return queue.client.updateDelay(keys.concat(args)).then(result => {
      if (result === 0) {
        queue.updateDelayTimer();
      } 
      return result;
    });
  },
